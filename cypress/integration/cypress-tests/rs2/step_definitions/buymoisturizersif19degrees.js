        ...
        if (temperature < 19) {
          ...
          cy.get(".font-weight-bold")
            ...
            .then((productName) => {
              ...
              cy.contains("span", "Pay with Card").should("be.visible").click();
  
              //   // Interact with Stripe's iframe to fill in card details
              //   cy.frameLoaded('cypress-iframe[name*="stripe_checkout_app"]'); // Ensure iframe is loaded
              //   cy.iframe('cypress-iframe[name*="stripe_checkout_app"]').within(
              //     () => {
              //       // Fill in card details
              //       cy.get(".emailInput.input") // Select the input field by ID
              //         .type("test@example.com", { delay: 100 }); // Type email with a slight delay between keystrokes
              ...
              //   cy.contains("PAYMENT SUCCESS").should("be.visible"); // Replace with actual confirmation text
            });
        }
      });
    });
